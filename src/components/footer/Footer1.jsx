        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
                ...
                <div className="mt-4 space-y-4">
                  {teamMembers.map(member => (
                    <div key={member.name} className="flex items-center space-x-3">
                      <img class="w-12 h-12 rounded-full object-cover" alt={member.name} src="https://images.unsplash.com/photo-1595872018818-97555653a011" loading="lazy" />
                      <div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }} viewport={{ once: true }}>
            ...
            <div className="flex space-x-4">
              <Button onClick={showToast} variant="ghost" size="icon" className="bg-white/10 hover:bg-white/20" aria-label="Facebook"><Facebook className="w-5 h-5 text-white" /></Button>
              <Button onClick={showToast} variant="ghost" size="icon" className="bg-white/10 hover:bg-white/20" aria-label="Twitter"><Twitter className="w-5 h-5 text-white" /></Button>
              <Button onClick={showToast} variant="ghost" size="icon" className="bg-white/10 hover:bg-white/20" aria-label="LinkedIn"><Linkedin className="w-5 h-5 text-white" /></Button>
              <Button onClick={showToast} variant="ghost" size="icon" className="bg-white/10 hover:bg-white/20" aria-label="Instagram"><Instagram className="w-5 h-5 text-white" /></Button>
            </div>
          </motion.div>
